...
/* for ch in stdin
buf append ch
if ch is the start of a token
    index = len buf

    keep an index to the start of each type of: key/val/new/end/etc

    tok key string intern offset index (do not store leading `"`)
    tok val string intern offset index (store leading `"`) */

use formatter::{print_formatted, FormatConfig};
use lexer::*;
use query::*;
